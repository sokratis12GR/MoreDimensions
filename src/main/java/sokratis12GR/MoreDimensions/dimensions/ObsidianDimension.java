...
public class ObsidianDimension
{
	...
	static
	{
		...
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("obsidianDimension_portal"));
		block = (ModTrigger) (new ModTrigger()
				.setUnlocalizedName("obsidianDimension_trigger")/* .setTextureName("ObsidianIgnator") */);
		// Item.itemRegistry.addObject(444, "obsidianDimension_trigger", block);
	}

	public ObsidianDimension()
	{
	}

	public void load(FMLInitializationEvent event)
	{
		if (ConfigHandler.enableObsidianDimension)
		{
			GameRegistry.registerBlock(portal, "obsidianDimension_portal");
			GameRegistry.registerItem(block, "obsidianDimension_trigger");
			DimensionManager.registerProviderType(DIMID, ObsidianDimension.WorldProviderMod.class, false);
			DimensionManager.registerDimension(DIMID, DIMID);

			if (event.getSide() == Side.CLIENT)
				Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(block, 0,
						new ModelResourceLocation("moredimensions:obsidianDimension_trigger", "inventory"));

			GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]
			{ "XXX", "XXX", "X78", Character.valueOf('7'), new ItemStack(Blocks.obsidian, 1), Character.valueOf('8'),
	...
	public static class WorldProviderMod extends WorldProvider
	{
		...
		{
			this.worldChunkMgr = new WorldChunkManagerHell(ObsidianWorld.biome, 0.0F);
		...
		{
			return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed() - 9277);
		}
	...
	public static class BlockTutorialPortal extends Block
	{
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
		{
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.obsidian
					|| getBlock(par1World, par2 + 1, par3, par4) == Blocks.obsidian)
			{
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.obsidian
					|| getBlock(par1World, par2, par3, par4 + 1) == Blocks.obsidian)
			{
				b1 = 1;
			}
			if (b0 == b1)
		...
		public void onNeighborBlockChange(World par1World, BlockPos pos, IBlockState state, Block neighborBlock)
		{
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.obsidian)
			{
			...
			} else
			{
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.obsidian)
				{
					...
					{
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.obsidian
								|| getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.obsidian
										|| getBlock(par1World, par2 + b0, par3, par4 + b1) != this))
						{
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
