...
public class DiamondDimension
{
	...
	static
	{
		...
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("diamondDimension_portal"));
		block = (ModTrigger) (new ModTrigger()
				.setUnlocalizedName("diamondDimension_trigger")/* .setTextureName("DiamondIgnator") */);
		// Item.itemRegistry.addObject(438, "diamondDimension_trigger", block);
	}
	...
	{
		if (ConfigHandler.enableDiamondDimension)
		{
			GameRegistry.registerBlock(portal, "diamondDimension_portal");
			GameRegistry.registerItem(block, "diamondDimension_trigger");
			DimensionManager.registerProviderType(DIMID, DiamondDimension.WorldProviderMod.class, false);
			DimensionManager.registerDimension(DIMID, DIMID);

			if (event.getSide() == Side.CLIENT)
				Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(block, 0,
						new ModelResourceLocation("moredimensions:diamondDimension_trigger", "inventory"));

			GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]
			{ "XXX", "XXX", "X78", Character.valueOf('7'), new ItemStack(Blocks.diamond_block, 1),
	...
	public static class WorldProviderMod extends WorldProvider
	{
		...
		{
			this.worldChunkMgr = new WorldChunkManagerHell(DiamondWorld.biome, 0.0F);
		...
		{
			return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed() - 3302);
		}
				...
				for (k3 = 0; k3 < 4; ++k3)
				{
					for (l3 = -1; l3 < 4; ++l3)
					{
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4),
								flag1 ? Blocks.diamond_block.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block
	{
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
		{
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.diamond_block
					|| getBlock(par1World, par2 + 1, par3, par4) == Blocks.diamond_block)
			{
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.diamond_block
					|| getBlock(par1World, par2, par3, par4 + 1) == Blocks.diamond_block)
			{
				b1 = 1;
			}
			if (b0 == b1)
		...
		public void onNeighborBlockChange(World par1World, BlockPos pos, IBlockState state, Block neighborBlock)
		{
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.diamond_block)
			{
			...
			} else
			{
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.diamond_block)
				{
					...
					{
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.diamond_block
								|| getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.diamond_block
										|| getBlock(par1World, par2 + b0, par3, par4 + b1) != this))
						{
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
