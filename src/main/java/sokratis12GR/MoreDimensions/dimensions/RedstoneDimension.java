...
public class RedstoneDimension {
...
portal = (BlockTutorialPortal)(new BlockTutorialPortal().setUnlocalizedName("redstoneDimension_portal"));
block = (ModTrigger)(new ModTrigger().setUnlocalizedName("redstoneDimension_trigger")/*.setTextureName("RedstoneIgnator")*/);
//Item.itemRegistry.addObject(434, "redstoneDimension_trigger", block);
}

public RedstoneDimension(){}

public void load(FMLInitializationEvent event){
	if (ConfigHandler.enableRedstoneDimension) {
GameRegistry.registerBlock(portal, "redstoneDimension_portal");
GameRegistry.registerItem(block, "redstoneDimension_trigger");
DimensionManager.registerProviderType(DIMID, RedstoneDimension.WorldProviderMod.class, false);
DimensionManager.registerDimension(DIMID, DIMID);

if(event.getSide() == Side.CLIENT)
Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(block, 0, new ModelResourceLocation("moredimensions:redstoneDimension_trigger", "inventory"));


GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]{
	"XXX", "XXX", "X78", Character.valueOf('7'), new ItemStack(Blocks.redstone_block, 1), Character.valueOf('8'), new ItemStack(Items.flint, 1), 
});
...
public static class WorldProviderMod extends WorldProvider{
    ...
    public void registerWorldChunkManager(){
        this.worldChunkMgr = new WorldChunkManagerHell(RedstoneWorld.biome, 0.0F);
    ...
    public IChunkProvider createChunkGenerator(){
        return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed()-11025);
    }
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? Blocks.redstone_block.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        boolean flag = k3 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? Blocks.redstone_block.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
        }
        ...
        for (j3 = 0; j3 < 4; ++j3)
        {
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.redstone_block.getDefaultState() : iblockstate, 2);
                }
            }
...
byte b0 = 0;
byte b1 = 0;
if (getBlock(par1World,par2 - 1, par3, par4) == Blocks.redstone_block || getBlock(par1World,par2 + 1, par3, par4) == Blocks.redstone_block)
{
b0 = 1;
}
if (getBlock(par1World,par2, par3, par4 - 1) == Blocks.redstone_block || getBlock(par1World,par2, par3, par4 + 1) == Blocks.redstone_block)
{
b1 = 1;
}
if (b0 == b1)
...
if (getBlock(par1World,par2, i1 - 1, par4) != Blocks.redstone_block)
{
...
if (j1 == 3 && getBlock(par1World,par2, i1 + j1, par4) == Blocks.redstone_block)
{
...
{
         if ((getBlock(par1World,par2 + b0, par3, par4 + b1) != Blocks.redstone_block || getBlock(par1World,par2 - b0, par3, par4 - b1) != this) && (getBlock(par1World,par2 - b0, par3, par4 - b1) != Blocks.redstone_block || getBlock(par1World,par2 + b0, par3, par4 + b1) != this))
         {
         par1World.setBlockToAir(new BlockPos(par2, par3, par4));
         }
}
...
public static class ChunkProviderModded implements IChunkProvider
{
                    ...
                    for (int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        for (int i3 = 0; i3 < 4; ++i3)
                        {
                            ...
                            for (int k3 = 0; k3 < 4; ++k3)
                            {
                                if ((d15 += d16) > 0.0D)
                                {
                                    p_147424_3_[j3 += short1] = Blocks.redstone_block;
                                }
                                else if (k2 * 8 + l2 < b0)
                                {
                                    p_147424_3_[j3 += short1] = Blocks.redstone_block;
	...
	public List<?> func_177458_a(EnumCreatureType p_177458_1_, BlockPos p_177458_2_)
    {
        ...
        if (this.mapFeaturesEnabled)
        {
            ...
            if (p_177458_1_ == EnumCreatureType.MONSTER &&  this.oceanMonumentGenerator.func_175796_a(this.worldObj, p_177458_2_))
            {
                    ...
                    for (int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        for (int i3 = 0; i3 < 4; ++i3)
                        {
                            ...
                            for (int j3 = 0; j3 < 4; ++j3)
                            {
                                if ((d15 += d16) > 0.0D)
                                {
                                    p_180518_3_.setBlockState(k * 4 + i3, k2 * 8 + l2, j1 * 4 + j3, Blocks.redstone_ore.getDefaultState());
                                }
                                else if (k2 * 8 + l2 < 63)
                                {
                                    p_180518_3_.setBlockState(k * 4 + i3, k2 * 8 + l2, j1 * 4 + j3, Blocks.redstone_block.getDefaultState());
                                }
                            }
