...
public class WoodDimension
{
	...
	static
	{
		...
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("woodDimension_portal"));
		block = (ModTrigger) (new ModTrigger()
				.setUnlocalizedName("woodDimension_trigger")/* .setTextureName("WoodIgnator") */);
		// Item.itemRegistry.addObject(439, "woodDimension_trigger", block);
	}

	public WoodDimension()
	{
	}

	public void load(FMLInitializationEvent event)
	{
		if (ConfigHandler.enableWoodDimension)
		{
			GameRegistry.registerBlock(portal, "woodDimension_portal");
			GameRegistry.registerItem(block, "woodDimension_trigger");
			DimensionManager.registerProviderType(DIMID, WoodDimension.WorldProviderMod.class, false);
			DimensionManager.registerDimension(DIMID, DIMID);

			if (event.getSide() == Side.CLIENT)
				Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(block, 0,
						new ModelResourceLocation("moredimensions:woodDimension_trigger", "inventory"));

			GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]
			{ "XXX", "XXX", "X78", Character.valueOf('7'), new ItemStack(Blocks.planks, 1), Character.valueOf('8'),
	...
	public static class WorldProviderMod extends WorldProvider
	{
		...
		{
			this.worldChunkMgr = new WorldChunkManagerHell(WoodWorld.biome, 0.0F);
		...
		{
			return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed() - 23152);
		}
				...
				for (k3 = 0; k3 < 4; ++k3)
				{
					for (l3 = -1; l3 < 4; ++l3)
					{
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4),
								flag1 ? Blocks.planks.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block
	{
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
		{
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.planks
					|| getBlock(par1World, par2 + 1, par3, par4) == Blocks.planks)
			{
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.planks
					|| getBlock(par1World, par2, par3, par4 + 1) == Blocks.planks)
			{
				b1 = 1;
			}
			if (b0 == b1)
		...
		public void onNeighborBlockChange(World par1World, BlockPos pos, IBlockState state, Block neighborBlock)
		{
			...
			} else
			{
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.planks)
				{
					...
					{
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.planks
								|| getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.planks
										|| getBlock(par1World, par2 + b0, par3, par4 + b1) != this))
						{
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
						...
						for (int l2 = 0; l2 < 8; ++l2)
						{
							...
							for (int i3 = 0; i3 < 4; ++i3)
							{
								...
								for (int k3 = 0; k3 < 4; ++k3)
								{
									if ((d15 += d16) > 0.0D)
									{
										p_147424_3_[j3 += short1] = Blocks.log;
									} else if (k2 * 8 + l2 < b0)
									{
										p_147424_3_[j3 += short1] = Blocks.log;
									} else
