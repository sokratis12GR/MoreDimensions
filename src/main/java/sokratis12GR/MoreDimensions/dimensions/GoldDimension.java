...
public class GoldDimension
{
	...
	static
	{
		...
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("goldDimension_portal"));
		block = (ModTrigger) (new ModTrigger()
				.setUnlocalizedName("goldDimension_trigger")/* .setTextureName("GoldIgnator") */);
		// Item.itemRegistry.addObject(436, "goldDimension_trigger", block);
	}

	public GoldDimension()
	{
	}

	public void load(FMLInitializationEvent event)
	{
		if (ConfigHandler.enableGoldDimension)
		{
			GameRegistry.registerBlock(portal, "goldDimension_portal");
			GameRegistry.registerItem(block, "goldDimension_trigger");
			DimensionManager.registerProviderType(DIMID, GoldDimension.WorldProviderMod.class, false);
			DimensionManager.registerDimension(DIMID, DIMID);

			if (event.getSide() == Side.CLIENT)
				Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(block, 0,
						new ModelResourceLocation("moredimensions:goldDimension_trigger", "inventory"));

			GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]
			{ "XXX", "XXX", "X78", Character.valueOf('7'), new ItemStack(Blocks.gold_block, 1), Character.valueOf('8'),
	...
	public static class WorldProviderMod extends WorldProvider
	{
		...
		{
			this.worldChunkMgr = new WorldChunkManagerHell(GoldWorld.biome, 0.0F);
		...
		{
			return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed() - 18052);
		}
				...
				for (k3 = 0; k3 < 4; ++k3)
				{
					for (l3 = -1; l3 < 4; ++l3)
					{
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4),
								flag1 ? Blocks.gold_block.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block
	{
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
		{
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.gold_block
					|| getBlock(par1World, par2 + 1, par3, par4) == Blocks.gold_block)
			{
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.gold_block
					|| getBlock(par1World, par2, par3, par4 + 1) == Blocks.gold_block)
			{
				b1 = 1;
			}
			if (b0 == b1)
		...
		public void onNeighborBlockChange(World par1World, BlockPos pos, IBlockState state, Block neighborBlock)
		{
			...
			} else
			{
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.gold_block)
				{
					...
					{
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.gold_block
								|| getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.gold_block
										|| getBlock(par1World, par2 + b0, par3, par4 + b1) != this))
						{
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
