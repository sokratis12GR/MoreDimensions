...
public class CoalDimension
{
	...
	static
	{
		...
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("coalDimension_portal"));
		block = (ModTrigger) (new ModTrigger()
				.setUnlocalizedName("coalDimension_trigger")/* .setTextureName("CoalIgnator") */);
		// Item.itemRegistry.addObject(433, "coalDimension_trigger", block);
	}

	public CoalDimension()
	{
	}
	...
	public static class WorldProviderMod extends WorldProvider
	{
		...
		{
			return new ChunkProviderModded(this.worldObj, this.worldObj.getSeed() - 11335);
		}
					...
					for (int i1 = -2; i1 <= 2; ++i1)
					{
						for (int j1 = -1; j1 < 3; ++j1)
						{
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
									flag ? Blocks.coal_block.getDefaultState() : Blocks.air.getDefaultState());
						}
					}
				}
				...
				for (k3 = 0; k3 < 4; ++k3)
				{
					for (l3 = -1; l3 < 4; ++l3)
					{
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4),
								flag1 ? Blocks.coal_block.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block
	{
		...
		public void onNeighborBlockChange(World par1World, BlockPos pos, IBlockState state, Block neighborBlock)
		{
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.coal_block)
			{
			...
			} else
			{
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.coal_block)
				{
					...
					{
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.coal_block
								|| getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.coal_block
										|| getBlock(par1World, par2 + b0, par3, par4 + b1) != this))
						{
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
						...
						for (int l2 = 0; l2 < 8; ++l2)
						{
							...
							for (int i3 = 0; i3 < 4; ++i3)
							{
								...
								for (int k3 = 0; k3 < 4; ++k3)
								{
									if ((d15 += d16) > 0.0D)
									{
										p_147424_3_[j3 += short1] = Blocks.coal_block;
									} else if (k2 * 8 + l2 < b0)
									{
										p_147424_3_[j3 += short1] = Blocks.coal_block;
									} else
